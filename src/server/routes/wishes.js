...
router.get('/random', (req, res) => {
  ...
  const rows = db.prepare('SELECT w.id, w.content, w.creator_genders, w.creator_orientations, w.contacts, w.wishmail_enabled, w.image_id FROM wishes w LEFT JOIN users u ON w.user_id = u.id WHERE w.is_active = 1 AND (u.id IS NULL OR u.is_active = 1) ORDER BY RANDOM() LIMIT ?').all(limit);
  res.json(
    rows.map((wish) => ({
      id: wish.id,
...
router.post('/:id/reactivate', async (req, res) => {
  ...
  const wish = db.prepare('SELECT id, content, creator_genders, creator_orientations, contacts, wishmail_enabled, image_id FROM wishes WHERE id = ?').get(id);
