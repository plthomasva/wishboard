...
describe('AwsMetricsDashboard', () => {
  ...
  it('shows empty-state message when groups array is empty after fetch', async () => {
    mockOk(response([]));
    await act(async () => { render(<AwsMetricsDashboard authHeader={AUTH} />); });
  ...
  it('triggers another fetch when Refresh Now is clicked', async () => {
    ...
    const spy = vi.fn().mockResolvedValue({ ok: true, json: () => Promise.resolve(response([])) } as any);
  ...
  it('renders chart (no "No data") when at least one point is non-zero', async () => {
    const data = response([
      group('Lambda', [series('duration', 'Duration (ms)', [
  ...
  it('formatValue: renders 1 MB for 1048576 bytes (counter sum)', async () => {
    const data = response([group('CF', [series('bytes', 'Bytes Downloaded', [pt('2024-01-01T12:00:00Z', 1_048_576)])])]);
    mockOk(data);
    await act(async () => { render(<AwsMetricsDashboard authHeader={AUTH} />); });
  ...
  it('formatValue: renders GB for values >= 1 GiB', async () => {
    const data = response([group('CF', [series('bytes', 'Bytes Downloaded', [pt('2024-01-01T12:00:00Z', 2_147_483_648)])])]);
    mockOk(data);
    await act(async () => { render(<AwsMetricsDashboard authHeader={AUTH} />); });
  ...
  it('formatValue: renders KB for values >= 1024 and < 1 MiB', async () => {
    const data = response([group('CF', [series('bytes', 'Bytes Downloaded', [pt('2024-01-01T12:00:00Z', 2048)])])]);
    mockOk(data);
    await act(async () => { render(<AwsMetricsDashboard authHeader={AUTH} />); });
  ...
  it('formatValue: renders raw B for values < 1024', async () => {
    const data = response([group('CF', [series('bytes', 'Bytes Downloaded', [pt('2024-01-01T12:00:00Z', 512)])])]);
    mockOk(data);
    await act(async () => { render(<AwsMetricsDashboard authHeader={AUTH} />); });
  ...
  it('formatValue: renders ms suffix for (ms) label', async () => {
    const data = response([group('Lambda', [series('duration', 'Duration (ms)', [pt('2024-01-01T12:00:00Z', 250)])])]);
    mockOk(data);
    await act(async () => { render(<AwsMetricsDashboard authHeader={AUTH} />); });
  ...
  it('formatValue: renders % suffix for (%) label', async () => {
    const data = response([group('CF', [series('cache', 'Cache Hit Rate (%)', [pt('2024-01-01T12:00:00Z', 98.5)])])]);
    mockOk(data);
    await act(async () => { render(<AwsMetricsDashboard authHeader={AUTH} />); });
  ...
  it('formatValue: renders "0" for zero value on gauge metric', async () => {
    const data = response([group('Lambda', [series('concurrent', 'Concurrent Executions', [pt('2024-01-01T12:00:00Z', 0)])])]);
    mockOk(data);
    await act(async () => { render(<AwsMetricsDashboard authHeader={AUTH} />); });
  ...
  it('formatValue: renders integer string without decimal for whole numbers', async () => {
    const data = response([group('Lambda', [series('concurrent', 'Concurrent Executions', [pt('2024-01-01T12:00:00Z', 7)])])]);
    mockOk(data);
    await act(async () => { render(<AwsMetricsDashboard authHeader={AUTH} />); });
  ...
  it('formatValue: renders one-decimal float for non-integer gauge', async () => {
    ...
    const data = response([group('Lambda', [series('latency_p99', 'P99 Latency', [pt('2024-01-01T12:00:00Z', 3.5)])])]);
    mockOk(data);
    await act(async () => { render(<AwsMetricsDashboard authHeader={AUTH} />); });
  ...
  it('shows "last hour total" note for counter-type metrics', async () => {
    const data = response([group('Lambda', [series('invocations', 'Invocations', [pt('2024-01-01T12:00:00Z', 100)])])]);
    mockOk(data);
    await act(async () => { render(<AwsMetricsDashboard authHeader={AUTH} />); });
  ...
  it('shows "latest" note for gauge-type metrics', async () => {
    const data = response([group('Lambda', [series('concurrent', 'Concurrent Executions', [pt('2024-01-01T12:00:00Z', 5)])])]);
    mockOk(data);
    await act(async () => { render(<AwsMetricsDashboard authHeader={AUTH} />); });
  ...
  it('currentValue: returns 0 when all dataPoints are 0 (gauge → "0")', async () => {
    const data = response([group('Lambda', [series('concurrent', 'Concurrent Executions', [
