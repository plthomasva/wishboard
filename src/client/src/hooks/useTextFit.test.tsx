...
describe('useTextFit', () => {
  it('scales down font size and calls ResizeObserver', () => {
    ...
    const TestComponent = () => {
      const { containerRef, contentRef } = useTextFit({ minFontSize: 10, maxFontSize: 20, step: 2 });
      
      // Mock overflowing properties
      if (containerRef.current) {
        Object.defineProperty(containerRef.current, 'scrollHeight', { value: 200, configurable: true });
        Object.defineProperty(containerRef.current, 'clientHeight', { value: 100, configurable: true });
      }

      return (
        <div ref={containerRef as any} data-testid="container">
          <div ref={contentRef as any} data-testid="content">Long text</div>
        </div>
      );
    };
