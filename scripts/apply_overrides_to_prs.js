...
const overrides = [
  ...
  { number: 382, conventionalMessage: 'refactor: simplify AppContent to improve code health' },
  {
    number: 381,
  ...
  { number: 377, conventionalMessage: 'refactor: extract SystemLogsSection component' },
  {
    number: 376,
  ...
  { number: 370, conventionalMessage: 'refactor: refactor LocalMetricsDashboard' },
  { number: 369, conventionalMessage: 'fix: prevent command injection in kiosk remote cleanup' },
  { number: 367, conventionalMessage: 'test: remove resolved issue comment from WiFiQrCode test' },
  { number: 366, conventionalMessage: 'fix: n+1 query issue during legacy SQLite migration' },
  {
    number: 365,
  ...
  { number: 363, conventionalMessage: 'refactor: simplify setupCamera function in WishScanner' },
  { number: 362, conventionalMessage: 'refactor: simplify renderOverlay in WishScanner' },
  {
    number: 361,
  ...
  { number: 358, conventionalMessage: 'refactor: simplify AwsMetricsDashboard component' },
  {
    number: 357,
...
for (const ov of overrides) {
  try {
    ...
    const prJson = execSync(
      `curl -s -H "Authorization: token ${process.env.GH_TOKEN}" -H "Accept: application/vnd.github.v3+json" https://api.github.com/repos/plthomasva/wishboard/pulls/${ov.number}`
    ).toString();
    ...
    // Update PR body
    fs.writeFileSync('temp_body.json', JSON.stringify({ body: newBody }));

    execSync(
      `curl -s -X PATCH -H "Authorization: token ${process.env.GH_TOKEN}" -H "Accept: application/vnd.github.v3+json" -d @temp_body.json https://api.github.com/repos/plthomasva/wishboard/pulls/${ov.number}`
    );
